...
function destroy(id, bulk) {
  ...
  bulkForm.post(route("bulk.delete.role"), {
    onSuccess: () => {
      success.value = true
      ids.value = []
    },
...
const checkAll = computed({
  get() {
    return props.roles.data ? ids.value.length == props.roles.data.length : false;
  },
  set(value) {
    var checked = [];
    if (value) {
      props.roles.data.forEach(function (role) {
        checked.push(role.id);
      });
    }
    ids.value = checked;
  }
            ...
            <div class="nk-block-between">
              <div class="nk-block-head-content">
                ...
                <div class="nk-block-des text-soft">
                  <p>You have total {{ roles.total }} roles.</p>
                </div>
              </div>
              <TopSortRight label="Add role" :href="route('role.create')" create />
            </div>
          </div>
                ...
                <template v-if="roles.data.length">
                  <div class="card-inner p-0">
                    <div class="nk-tb-list nk-tb-ulist">
                      <div class="nk-tb-item nk-tb-head">
                        <div class="nk-tb-col nk-tb-col-check">
                          <div class="custom-control custom-control-sm custom-checkbox notext">
                            <input type="checkbox" class="custom-control-input" v-model="checkAll" id="rid">
                            <label class="custom-control-label" for="rid"></label>
                          </div>
                        </div>
                      ...
                      <div v-for="role in roles.data" :key="role.id" class="nk-tb-item">
                        <div class="nk-tb-col nk-tb-col-check">
                          <div class="custom-control custom-control-sm custom-checkbox notext">
                            <input type="checkbox" class="custom-control-input" v-model="ids" :id="`rid` + role.id"
                              :value="role.id">
                            <label class="custom-control-label" :for="`rid` + role.id"></label>
                          </div>
                        </div>
                        <div class="nk-tb-col">
                        ...
                        <div class="nk-tb-col"><span>{{ role.description }} </span></div>
                          ...
                          <ul class="nk-tb-actions gx-1">
                              ...
                              <CTooltip content="Move to Trash" placement="top">
                                <template #toggler="{ on }">
                                  <a href="#" class="btn btn-trigger btn-icon" v-on="on" aria-label="Move to Trash"
                                    @click.prevent="confirmModal(role.id)">
                            ...
                            <li class="nk-tb-action-hidden">
                              <CTooltip content="Edit Role" placement="top">
                                <template #toggler="{ on }">
                                  <Link :href="route('role.edit', role.id)" class="btn btn-trigger btn-icon" v-on="on"
                            ...
                            <li>
                              <div class="drodown">
                                <a class="dropdown-toggle btn btn-icon btn-trigger" data-bs-toggle="dropdown"
                                  :data-dropdown-toggle="`tb-action-r-` + role.id"
                                  data-dropdown-placement="bottom-end"><em class="icon ni ni-more-h"></em></a>
                                <div class="dropdown-menu dropdown-menu-end" :id="`tb-action-r-` + role.id">
                                  ...
                                  <ul class="link-list-opt no-bdr">
                                    ...
                                    <li class="divider"></li>
                                    <li>
                                      <a href="#" @click.prevent="confirmModal(role.id)">
                                        <em class="icon ni ni-trash"></em>
                  ...
                  <CustomPagination :data="roles" />
                </template>
                <template v-else>
                  <div class="card-inner p-0">
                    <NoRecord label="roles" />
                  </div>
                </template>
              </div>
            </div>
