...
class ChestXRaysController extends Controller
{
    ...
    {
        $ChestXRay = ChestXRay::where('patient_id', $id)->first();
        $patient = Patient::find($id);
        if(!$ChestXRay){
            abort(404);
        }
        return Inertia::render('Admin/ChestXRay/Edit', [
            'chest_x_ray' => $ChestXRay,
            'patient' => $patient
        ]);
    }
     ...
     */
    public function update(Request $request, ChestXRay $ChestXRay)
    {
        if ($ChestXRay->update($request->all())) {
            $patient = Patient::find($ChestXRay->patient_id);
