...
class SkinTegumentsController extends Controller
{
    ...
    {
        $skinTeguments = SkinTeguments::where('patient_id', $id)->first();
        $patient = Patient::find($id);
        if(!$skinTeguments){
            abort(404);
        }
        
        return Inertia::render('Admin/SkinTeguments/Edit', [
            'skin_teguments' => $skinTeguments,
            'patient' => $patient
        ]);
    }
    ...
    {
        $skinTeguments = SkinTeguments::find($id);
        if ($skinTeguments->update($request->all())) {
            $patient = Patient::find($skinTeguments->patient_id);
