...
class PastMedicalHistoryController extends Controller
{
    ...
    {
        $pastMedicalHistory = PastMedicalHistory::where('patient_id', $id)->first();
        $patient = Patient::find($id);
        if(!$pastMedicalHistory){
            abort(404);
        }
        return Inertia::render('Admin/PastMedicalHistory/Edit', [
            'medical_history' => $pastMedicalHistory,
            'patient' => $patient
        ]);
    }
     ...
     */
    public function update(Request $request, PastMedicalHistory $pastMedicalHistory)
    {
        if ($pastMedicalHistory->update($request->all())) {
            $patient = Patient::find($pastMedicalHistory->patient_id);
