...
class GeneralAppearanceController extends Controller
{
    ...
    {
        $generalAppearance = GeneralAppearance::where('patient_id', $id)->first();
        $patient = Patient::find($id);
        if(!$generalAppearance){
            abort(404);
        }
        return Inertia::render('Admin/GeneralAppearance/Edit', [
     ...
     */
    public function update(Request $request, GeneralAppearance $generalAppearance)
    {
        if ($generalAppearance->update($request->all())) {
            $patient = Patient::find($generalAppearance->patient_id);
