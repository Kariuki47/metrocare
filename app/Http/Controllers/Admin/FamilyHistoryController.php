...
class FamilyHistoryController extends Controller
{
    ...
    {
        $FamilyHistory = FamilyHistory::where('patient_id', $id)->first();
        $patient = Patient::find($id);
        if(!$FamilyHistory){
            abort(404);
        }
        
        return Inertia::render('Admin/FamilyHistory/Edit', [
    ...
    {
        if ($FamilyHistory->update($request->all())) {
            $patient = Patient::find($FamilyHistory->patient_id);
