...
class SensoryOrganController extends Controller
{
    ...
    {
        $sensoryOrgan = SensoryOrgan::where('patient_id', $id)->first();
        $patient = Patient::find($id);
        if(!$sensoryOrgan){
            abort(404);
        }
        return Inertia::render('Admin/SensoryOrgan/Edit', [
            'sensory_organ' => $sensoryOrgan,
            'patient' => $patient
        ]);
    }
     ...
     */
    public function update(Request $request, SensoryOrgan $sensoryOrgan)
    {
        if ($sensoryOrgan->update($request->all())) {
            $patient = Patient::find($sensoryOrgan->patient_id);
