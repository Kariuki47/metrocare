...
class GastrointestinalSystemController extends Controller
{
    ...
    {
        $GastrointestinalSystem = GastrointestinalSystem::where('patient_id', $id)->first();
        $patient = Patient::find($id);
        if(!$GastrointestinalSystem){
            abort(404);
        }
        return Inertia::render('Admin/GastrointestinalSystem/Edit', [
    ...
    {
        $GastrointestinalSystem = GastrointestinalSystem::find($id);
        if ($GastrointestinalSystem->update($request->all())) {
            $patient = Patient::find($GastrointestinalSystem->patient_id);
